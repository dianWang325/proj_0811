#!/usr/bin/env python3
"""Validate CPP diagnostics and write extracted events and a structured result."""
...
SCHEDULER_FIELDS = {
    "iteration", "req_id", "num_computed_tokens", "hist_seq_len",
    "remaining_prefill_tokens", "target_latency_ms", "predicted_chunk_size",
    "actual_scheduled_chunk_size", "predicted_latency_ms",
    "actual_execution_time_ms", "with_history_ready", "history_fitted",
    "fit_sample_count", "predictor_updated", "disable_profiling_timing",
}
...
def main() -> None:
    ...
    result = {
        ...
        "scheduler_iteration_count": len(iterations),
        "scheduled_chunk_sizes": sorted(chunk_sizes),
    }
    args.result_output.parent.mkdir(parents=True, exist_ok=True)
    args.result_output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    print(
        "CPP_TRACE_VALIDATION_PASS "
        f"runner={args.runner} dynamic={args.dynamic} "
        f"iterations={len(iterations)} chunks={sorted(chunk_sizes)}"
    )
